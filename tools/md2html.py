...
def convierte(texto, titulo, actual, idioma="en"):
    ...
    return (f"<title>{html.escape(titulo)}</title>\n<style>{ESTILO}</style>\n"
            ...
            '\n<footer><p><em>Ale Hop!</em> lo publico Topo Soft en 1988; segun sus propios '
            'creditos, el programa es de Alberto L. Navarro y los graficos de '
            'LuigiLopez. Todos los derechos sobre el juego siguen siendo de sus '
            'titulares. Este trabajo es de preservacion, estudio y documentacion.'
            '</p></footer></div>\n')
...
def main(docdir, idioma="en"):
    ...
    for fn in sorted(os.listdir(docdir)):
        ...
        titulo = (m.group(1) if m else fn[:-3]) + " — Ale Hop! (1988)"
